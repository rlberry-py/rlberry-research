...
from rlberry_research.agents.torch.utils.training import (
    loss_function_factory,
    model_factory,
    optimizer_factory,
    size_model_config,
)
from rlberry_research.agents.torch.dqn.dqn_utils import (
    polynomial_schedule,
    lambda_returns,
)
from rlberry_research.agents.utils import replay
...
class DQNAgent(AgentTorch, AgentWithSimplePolicy):
    ...
    q_net_constructor : Callable, str or None
        ...
        * Ouput shape = (batch_dim, chunk_size, number_of_actions)
        ...
        Example: use `rlberry_research.agents.torch.utils.training.model_factory_from_env`,
         and `q_net_kwargs`
    ...
    q_net_constructor : Callable, str or None
        Function/constructor that returns a torch module for the Q-network.
        Example: use `rlberry_research.agents.torch.utils.training.model_factory_from_env` and `q_net_kwargs`
        parameter to modify the neural network.
    q_net_kwargs : optional, dict
        Parameters for `q_net_constructor`.
    use_double_dqn : bool, default: False
